...
export function snapshot() {
  return {
    ...
    partyConfetti: keys.has("KeyF"), // F 觸發彩帶
    partyMusic: keys.has("KeyY"), // Y 切換派對音樂狀態
    voidMode: keys.has("KeyX"), // 按住 X 進入無敵桌布模式
    voidThemeNext: keys.has("KeyZ"), // Z 切換桌布主題
    voidParticleUp: keys.has("KeyM"), // M 調整粒子量
    goalCelebrate: keys.has("Digit1"), // 1 慶祝
    goalNext: keys.has("Digit2"), // 2 下一關
    goalReplay: keys.has("Digit3"), // 3 重玩本關
    eventInteract: keys.has("KeyT"), // T 觸發事件房/祭壇
  };
}
