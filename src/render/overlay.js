...
import { HYPERDRIVE, VOID_MODE } from "../config.js";
...
export function drawAbilityHud(ctx, world) {
  ...
  if (world.celebration?.active) {
    ...
    ctx.fillText("慶祝模式：V 壁紙(按住) / C 分身 / F 彩帶 / N 離開", hudX, hudY + hudH + 42);
    if (world.celebration.wallpaperHeld) {
      ctx.fillText("壁紙 ON", hudX + hudW - 80, hudY - 4);
    }
    const musicOn = world.celebration?.musicOn;
    ctx.fillText(`音樂: ${musicOn ? "開" : "關"} (Y 切換)`, hudX, hudY + hudH + 56);
  }
  if (world.voidMode?.active) {
    ctx.fillStyle = "#a0e3ff";
    const themeIdx = world.voidMode.themeIndex || 0;
    const themeName =
      (VOID_MODE?.themes && VOID_MODE.themes[themeIdx]?.name) || "桌布";
    const mult = world.voidMode.particleMult || 1;
    ctx.fillText(`無敵桌布：X 按住 | Z 主題: ${themeName} | M 粒子 x${mult}`, hudX, hudY + hudH + 58);
    ctx.fillText("煙火鍵：J/L/U", hudX, hudY + hudH + 72);
  }

  if (world.goalHint?.active && !world.celebration?.active && !world.voidMode?.active) {
    ctx.fillStyle = "#b0ffb0";
    const dist = Math.round(world.goalHint.distance || 0);
    ctx.fillText(`靠近光道將進入慶祝！(剩餘 ${dist} px)`, hudX, hudY + hudH + 88);
    // HUD arrow indicator
    const arrowY = hudY + hudH + 102;
    const arrowX = hudX + 10;
    ctx.beginPath();
    ctx.moveTo(arrowX, arrowY - 4);
    ctx.lineTo(arrowX + 20, arrowY - 4);
    ctx.lineTo(arrowX + 20, arrowY - 10);
    ctx.lineTo(arrowX + 36, arrowY + 2);
    ctx.lineTo(arrowX + 20, arrowY + 14);
    ctx.lineTo(arrowX + 20, arrowY + 8);
  ...
  if (world.goalMenu?.active) {
    const ready = world.goalMenu.ready;
    const alpha = ready ? 0.6 : 0.4;
    ctx.fillStyle = `rgba(0,0,0,${alpha})`;
    ctx.fillRect(0, 0, ctx.canvas.width, ctx.canvas.height);
    ctx.fillStyle = ready ? "#ffffff" : "rgba(255,255,255,0.8)";
    ctx.font = "20px monospace";
    ctx.fillText("通關！ 1=慶祝  2=下一關  3=重玩", ctx.canvas.width / 2 - 180, ctx.canvas.height / 2);
    ctx.font = "12px monospace";
    ctx.fillText(ready ? "請按 1/2/3 選擇" : "請稍候...", ctx.canvas.width / 2 - 60, ctx.canvas.height / 2 + 20);
  }

  // 右下角戰鬥鍵提示
  ctx.font = "11px monospace";
  ctx.fillStyle = "rgba(200,200,200,0.75)";
  const keys = "攻擊 J/L/U/I/O/P/G | 盾 B | Hyper K/H | 桌布 X/Z/M/J/L/U | 派對 V/C/F/Y";
  const textWidth = ctx.measureText(keys).width;
  ctx.fillText(keys, ctx.canvas.width - textWidth - 14, ctx.canvas.height - 16);
