...
import { EventShrineSystem } from "./systems/event-shrine-system.js";
import { partyMusic } from "./audio/party-music.js";
...
import { ArtifactMenu } from "./ui/artifact-menu.js";
...
async function init() {
  ...
  loadingTimer = setInterval(() => {
    const p = assetLoader.getProgress();
    if (loadingFill) loadingFill.style.width = `${Math.round(p * 100)}%`;
    if (loadingText) loadingText.textContent = `載入中... ${Math.round(p * 100)}%`;
  }, 100);
  ...
  world.hitFlashTimer = 0;
  world.fragments = 0;
  world.passives = {};
  world.sceneOverlay = sceneOverlay;

  // Store infiniteWorld reference in world for debugging
  world.infiniteWorld = infiniteWorld;
  world.voidMode = { active: false, themeIndex: 0, particleMult: 1 };
  ...
  systems.eventShrine = new EventShrineSystem(world);
  ...
  systems.render.extraObjects = levelManager.currentDecor || [];
  ...
  // Create artifact menu
  artifactMenu = new ArtifactMenu();
  world.artifactMenu = artifactMenu;
  ...
  window.addEventListener("sfx", (e) => {
    if (e.detail?.type === "party-music-toggle") {
      partyMusic.toggle();
    }
  });
...
function gameLoop(now) {
  ...
  acc += scaledDt;
  if (world?.hitFlashTimer !== undefined) {
    world.hitFlashTimer = Math.max(0, world.hitFlashTimer - rawDt);
  }
  if (world?.sceneOverlay?.timer > 0) {
    world.sceneOverlay.timer = Math.max(0, world.sceneOverlay.timer - rawDt);
  }
  ...
  const goalMenuActive = Boolean(world.goalMenu?.active);
  ...
  if (voidActiveRender) {
    const theme = (VOID_MODE.themes || [])[world.voidMode.themeIndex || 0] || VOID_MODE.themes?.[0];
    const grd = ctx.createLinearGradient(0, 0, ctx.canvas.width, ctx.canvas.height);
    grd.addColorStop(0, theme?.wallpaperA || "rgba(0,15,30,0.9)");
    grd.addColorStop(1, theme?.wallpaperB || "rgba(10,0,20,0.8)");
    ctx.fillStyle = grd;
    ctx.fillRect(0, 0, ctx.canvas.width, ctx.canvas.height);
  } else if (isWallpaper) {
    const grd = ctx.createLinearGradient(0, 0, ctx.canvas.width, ctx.canvas.height);
  ...
  if (world.hitFlashTimer > 0) {
    const alpha = Math.max(0, Math.min(0.35, world.hitFlashTimer * 1.5));
    ctx.fillStyle = `rgba(255,80,80,${alpha})`;
    ctx.fillRect(0, 0, ctx.canvas.width, ctx.canvas.height);
  }

  if (world.sceneOverlay?.timer > 0 && world.sceneOverlay.text) {
    ctx.fillStyle = "rgba(0,0,0,0.35)";
    ctx.fillRect(0, 0, ctx.canvas.width, ctx.canvas.height);
    ctx.font = "22px monospace";
    ctx.fillStyle = "#ffffff";
    const text = world.sceneOverlay.text;
    const w = ctx.measureText(text).width;
    ctx.fillText(text, (ctx.canvas.width - w) / 2, ctx.canvas.height * 0.35);
  }
