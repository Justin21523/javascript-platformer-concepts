...
export class DamageSystem {
  ...
  applyDamage(event) {
    ...
    if (health) {
      ...
      if (victim === this.world.player) {
        this.world.hitFlashTimer = 0.2;
        if (typeof window !== "undefined") {
          window.dispatchEvent(new CustomEvent("camera-shake", { detail: { strength: 9, duration: 0.25 } }));
        }
      }
      if (blocked) {
        this.spawnBlockSpark(victim, attackerTransform);
        if (this.world.blockCounter && event.attacker) {
          const atkHealth = this.world.getComponent(event.attacker, "Health");
          if (atkHealth) {
            atkHealth.current = Math.max(0, atkHealth.current - 12);
          }
        }
      }
    }
    ...
    if (health && health.current <= 0) {
      ...
      const attackerTeam = this.world.getComponent(event.attacker, "Team");
      if (victimTeam?.id === "enemy" && attackerTeam?.id === "player") {
        const bonus = this.world.fragmentBonus ?? 0;
        const gain = 1 + Math.floor(Math.random() * 2) + bonus;
        this.world.fragments = (this.world.fragments || 0) + gain;
        const heal = this.world.healOnKill ?? 0;
        if (heal > 0) {
          const playerHealth = this.world.getComponent(attackerTeam ? event.attacker : this.world.player, "Health");
          if (playerHealth) {
            playerHealth.current = Math.min(playerHealth.max, playerHealth.current + heal);
          }
        }
      }
  ...
  spawnBlockSpark(victim, attackerTransform) {
    const t = this.world.getComponent(victim, "Transform");
    if (!t || !attackerTransform) return;
    const spark = this.world.createEntity();
    const x = (t.x + attackerTransform.x) * 0.5;
    const y = (t.y + attackerTransform.y) * 0.5;
    this.world.addComponent(spark, "Transform", { x, y, z: 0 });
    this.world.addComponent(spark, "AABB", { w: 22, h: 22, ox: 0, oy: 0 });
    this.world.addComponent(spark, "Renderable", {
      color: "rgba(255,255,200,0.85)",
      opacity: 0.9,
      renderDuringVoid: true,
    });
    this.world.addComponent(spark, "Projectile", { lifetime: 0.25, gravity: 0, growRate: 120, maxSize: 48 });
    this.world.addComponent(spark, "Vfx", { lifetime: 0.25, fade: true, initial: 0.25 });
  }
