  ...
  checkXCollision(entity, x, y, aabb) {
    ...
    for (const tile of tiles) {
      if (
        ...
        // MTV 修正 (簡化版)
        const centerX = x + aabb.ox + aabb.w / 2;
        const tileCenterX = tile.x + tile.w / 2;

        if (centerX < tileCenterX) {
          // 撞到右牆，推回左邊
          const transform = this.world.getComponent(entity, "Transform");
          transform.x = tile.x - aabb.w - aabb.ox - 1;
        } else {
          // 撞到左牆，推回右邊
          const transform = this.world.getComponent(entity, "Transform");
          transform.x = tile.x + tile.w - aabb.ox + 1;
        }
        return true;
      }
    }
  ...
  checkYCollision(entity, x, y, aabb) {
    ...
    for (const tile of tiles) {
      if (
        ...
        // MTV 修正
        const centerY = y + aabb.oy + aabb.h / 2;
        const tileCenterY = tile.y + tile.h / 2;

        if (centerY < tileCenterY) {
          // 撞到地面，推到上方
          const transform = this.world.getComponent(entity, "Transform");
          transform.y = tile.y - aabb.h - aabb.oy - 1;
        } else {
          // 撞到天花板，推到下方
          const transform = this.world.getComponent(entity, "Transform");
          transform.y = tile.y + tile.h - aabb.oy + 1;
        }
        return true;
      }
    }
