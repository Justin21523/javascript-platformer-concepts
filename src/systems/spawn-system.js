...
export class SpawnSystem {
  ...
  update(dt) {
    ...
    if (!playerT) return;
    if (this.isRestZone(playerT.x)) return;
  ...
  ensureAirBands(playerT, enemies) {
    ...
    for (let b = bandIndex - this.bandLookahead; b <= bandIndex + this.bandLookahead; b++) {
      ...
      for (let i = 0; i < needed && spawned < maxNew; i++, spawned++) {
        ...
        if (roll < 0.45) this.spawnFlyingNPC(playerT, b);
        else if (roll < 0.8) this.spawnSentinelNPC(playerT, b);
        else this.spawnSupportNPC(playerT, b);
      }
    }
  }
  ...
  spawnGroundNPC(playerT) {
    ...
    if (roll < 0.4) {
      ...
      createShieldBearer(this.world, { x, y });
    } else if (roll < 0.85) {
      createBombThrower(this.world, { x, y });
    } else {
      createBossBrute(this.world, { x, y });
    }
  }
  ...
  spawnSupportNPC(playerT, bandIndex = null) {
    const x = playerT.x + this.randRange(200, 480) * (Math.random() < 0.5 ? -1 : 1);
    if (this.isRestZone(x)) return;
    if (Math.abs(x - playerT.x) < this.safeRadius) return;
    const baseY = bandIndex !== null ? -(bandIndex * this.verticalStep + this.verticalStep * 0.5) : playerT.y;
    const y = baseY + this.randRange(-60, 60);
    createSupportMage(this.world, { x, y });
  }
  ...
  isRestZone(x) {
    const zones = this.world.restZones || [];
    return zones.some((z) => x >= z.startX && x <= z.endX);
  }
}
