...
export class RenderSystem {
  ...
  drawTiles(camera) {
    ...
    // Draw extra objects (decor images)
    if (this.infiniteWorld && this.extraObjects.length) {
      const visibleObjs = this.infiniteWorld.getVisibleObjects(
        camera.x,
        camera.y,
        RENDER.CANVAS_WIDTH,
      ...
      for (const obj of visibleObjs) {
        const img = assetLoader.getImage(obj.path);
        if (img) {
          const sx = obj.x - camera.x;
          const sy = obj.y - camera.y - img.height + this.infiniteWorld.tileSize * 0.5;
  ...
  drawEntity(entity, camera) {
    ...
    if (wave) {
      ...
      ctx.beginPath();
      ctx.arc(screenX + w * 0.5, screenY + h * 0.5, Math.max(w, h) * 0.5, 0, Math.PI * 2);
      ctx.fill();
