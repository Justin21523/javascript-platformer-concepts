...
export class LevelManager {
  constructor(tilemapLoader, infiniteWorld, generator, options = {}) {
    ...
    this.procCounter = 0;
    this.procThemes = options.procThemes || ["tileset", "plains", "ruins", "clouds"];
    this.procSize = {
      width: options.procWidth || WORLD.segmentWidth || 64,
      height: options.procHeight || WORLD.segmentHeight || 24,
    };
    this.procSegments = WORLD.segmentCountRange || [3, 5];
    this.currentGoalY = null;
  }
  ...
  getNextLevelId() {
    ...
    return this.ensureProceduralLevel();
  }
  ...
  async loadLevel(levelId, options = {}) {
    let level = this.levels.find((lvl) => lvl.id === levelId);
    if (!level && levelId?.startsWith("proc-auto-")) {
      level = this.buildProceduralLevelConfig();
    ...
    this.currentDecor = level.meta?.decorImages || [];
    this.restZones = level.meta?.restZones || [];
    this.currentGoalY = level.meta?.goalY || null;
  ...
  ensureProceduralLevel() {
    const cfg = this.buildProceduralLevelConfig();
    this.levels.push(cfg);
    return cfg.id;
  }

  buildProceduralLevelConfig() {
    const theme = this.procThemes[this.procCounter % this.procThemes.length] || "plains";
    const id = `proc-auto-${this.procCounter}`;
    this.procCounter += 1;
    const segments = this.randomSegmentCount();
    return {
      id,
      name: `Procedural ${theme} #${this.procCounter}`,
      procedural: {
        theme,
        width: this.procSize.width,
        height: this.procSize.height,
        seed: Date.now() ^ this.procCounter,
        segments,
      },
      autoProcedural: true,
    };
  ...
  randomSegmentCount() {
    const min = this.procSegments[0] || 3;
    const max = this.procSegments[1] || 5;
