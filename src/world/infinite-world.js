...
export class InfiniteWorld {
  ...
  /**
   * Render helper: allows extra image overlays (objects) without modifying tile layers
   */
  getVisibleObjects(cameraX, cameraY, viewWidth, viewHeight, objects = []) {
    const visible = [];
    const margin = 100;
    const minX = cameraX - margin;
    const maxX = cameraX + viewWidth + margin;
    const minY = cameraY - margin;
    const maxY = cameraY + viewHeight + margin;
